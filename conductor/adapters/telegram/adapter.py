...
Outbound: ConductorTask result → Telegram reply (TODO: implement send)
...
class ProjectExtractor:
    ...
    def __init__(self, llm: LLMPort, registry: ProjectRegistryPort) -> None:
        self._llm = llm
...
class TelegramAdapter(AdapterPort):
    ...
    def __init__(self, extractor: ProjectExtractor) -> None:
    ...
    async def to_task(self, payload: dict) -> ConductorTask:  # type: ignore[type-arg]
        ...
        project = await self._extractor.extract(message)
        return ConductorTask(
            id=str(uuid.uuid4()),
            external_id=str((payload.get("message") or {}).get("message_id", "")),
            source="telegram",
            title=message[:80] if message else "(no message)",
            spec=message,
    ...
    async def from_task(self, task: ConductorTask) -> None:
        """Post the task result back to the Telegram chat. (Not yet implemented.)"""
        pass
